  ...
  const paint = () => {
    ...
    gateCounts(cases).forEach((gate, i) => {
      g.appendChild(el(`<div class="gate"><div class="n">${i + 1}</div>
        ...
        <div class="c" data-kpi-def="dqaf">${gate.checked} ${s.ctrlChecked || 'live cases'}<br><b class="${gate.held ? 'warn' : 'good'}" data-kpi-def="dqaf">${gate.held} ${s.ctrlHeldCycle || 'held this cycle'}</b></div></div>`));
    });
    if (!cases.length) {
